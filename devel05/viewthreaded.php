...
if($postnum=="0") {
        ...
        $result2 = $stream->do_query("select poster_id, post, time, ip, sig from evo_posts_$forumid where post_id = '$postnum'", "row");
...
<td colspan=3 align=center><b><?php $style->textout(wordfilter($subject)); ?> &nbsp; <?php echo $msgicon; ?></b></td>
...
$tmp = $stream->do_query("select * from evo_posts_$forumid where topic_id = '$topicid' order by time ASC", "row");
...
if($hppp=="0"){
   ...
   $homepagep = "<a href=\"$hppp\"><img src=images/www_sm.gif border=0></a>&nbsp;";
        ...
        <td width=150 valign=top bgcolor=<?php echo $style->tbl1color; ?>><?php $style->textout($username); ?><br>
        ...
        $language[post]s: $posts<br>



        $language[awards]: $awards<br>



        $language[views]: $views<br><br>
...
<?php echo "$icq$msnm$aim$email$homepage"; ?>
        ...
        <td align=left valign=top bgcolor=<?php echo $style->tbl1color; ?>>
                ...
                <?php $style->textout("<b>$language[Posted]:</b> $posttime&nbsp;$ago"); ?>
                ...
                <?php $style->textout("<b>$language[subject]:</b> $postsubject"); ?>
        ...
        <td width="10" valign=top align=center bgcolor=<?php echo $style->tbl1color; ?>>
...
<td colspan=2 width=100% align=center><?php $style->headertextout("<b>$language[posts]</b>"); ?></td>
...
$select = $stream->do_query("select post_id, poster_id, subject from evo_posts_$forumid where topic_id = '$topicid'", "array");
...
for($x=0; $x<count($select); $x++) {
        ...
        $thesubject = rawurldecode(rawurldecode(stripslashes($tmp[2])));
        ...
        <td bgcolor="<?php echo $style->tbl1color; ?>" width=90%>

        <?php

        if($x != 0) {

                $style->textout("&nbsp;&nbsp;<b>>></b> <a href=\"viewthreaded.php?groupid=$groupid&forumid=$forumid&topicid=$topicid&postnum=$postid\">$thesubject</a>");

        } else {

                $style->textout("<a href=\"viewthreaded.php?groupid=$groupid&forumid=$forumid&topicid=$topicid&postnum=$postid\">$subject</a>");
        ...
        <td bgcolor="<?php echo $style->tbl2color; ?>" width=10%>

        <?php $style->textout("<a href=\"members.php?do=profile&member=$poster_id\">$username</a>"); ?>
...
for($x=0; $x<count($groups); $x++) {
     ...
     echo "<option value=fdisplay.php?groupidx=$auto>$name</option>\n";
