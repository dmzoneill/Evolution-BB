...
$alluserscanpost = $rules[4];
        ...
        $forumname = ereg_replace("%20", " ", rawurldecode($stream->do_query("select title from evo_forums where auto = '$forumid'", "one")));
...
<td bgcolor="<?php echo $style->tbl1color; ?>" colspan=2><?php $style->textout("<a href=fdisplay.php>" .$language[forumindex] ."</a> > $forumname");?></td>
...
$tempid = $userid+10000000;
	...
	$result = $stream->do_query("INSERT INTO evo_posts_$forumid (topic_id, post_id, poster_id, post, time, something, notify, ip, sig, attachment) VALUES ('$topicid', '', '$userid', '$post', '$time', '', '$notify', '$ip', '$sig', '$crap')", "one");
        ...
        $postid = $stream->do_query("select post_id from evo_posts_$forumid where topic_id = '$topicid'", "one");
...
<?php $style->textout($language[newtopicdone] ."<br>");?>

<a href="<?php if ($threaded<1){echo "viewtopic.php";} else {echo "threadedview.php";}?><?php echo "?groupid=$groupid&forumid=$forumid&topicid=$topicid&postid=$postid";?>"><?php $style->textout($language[clickhere]);?></a>
...
setTimeout("document.location.href='<?php if ($threaded<1){echo "viewtopic.php";} else {echo "threadedview.php";}?>?groupid=<?php echo $groupid;?>&forumid=<?php echo $forumid;?>&topicid=<?php echo $topicid;?>&postid=<?php echo $postid;?>';", 2000);
...
<input type=hidden name=notify value="<?php echo $notify; ?>"><input type=hidden name=sig value="<?php echo $sig;?>"><input type=hidden name=audience value=<?php echo $audience; ?>><input type=hidden name=attachment value=<?php echo strtolower($fupload_name); ?>></td>
...
<td bgcolor="<?php echo $style->tbl_header; ?>" colspan=2 align=center><b><?php $style->headertextout($language[newtopic]); ?></b></td>
...
<?php $style->textout("<a href=fdisplay.php>" .$language[forumindex] ."</a> > $forumname"); ?>
...
<td bgcolor="<?php echo $style->tbl2color; ?>" align=right><?php $style->textout($language[selectauidence]); ?></td>

<td bgcolor="<?php echo $style->tbl1color; ?>"><select name="audience">

<option value="0" selected><?php echo $language[everybody]; ?></option>

<option value="1"><?php echo $language[modabove]; ?></option>

<option value="2"><?php echo $language[admins]; ?></option>
...
<td bgcolor="<?php echo $style->tbl1color; ?>"><input type=submit value="<?php echo $language[submit]; ?>">&nbsp;<input type=reset value="<?php echo $language[reset]; ?>"></td>
