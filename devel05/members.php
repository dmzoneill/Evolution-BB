...
<form method=post action=members.php?do=adduser&mode=dosignup name=signup>

<tr>

 <td width=30% bgcolor="<?php echo $style->tbl1color; ?>"><b><?php $style->textout($language[username]); ?></b> </td>

<td bgcolor="<?php echo $style->tbl2color; ?>"><input type=text name=u_name size=20 maxlength=20> <?php $style->textout($language[publicname]); ?></td>
						   ...
						   <tr>
                                   ...
                                   <td bgcolor="<?php echo $style->tbl1color; ?>"><b><?php $style->textout($language[birthday] ."</b>"); ?></td>
								...
								<select name=month>
								  <OPTION VALUE="January"><?php $style->textout($language[January]); ?></OPTION>
								  <OPTION VALUE="February"><?php $style->textout($language[February]); ?></OPTION>
								  <OPTION VALUE="March"><?php $style->textout($language[March]); ?></OPTION>
								  <OPTION VALUE="April"><?php $style->textout($language[April]); ?></OPTION>
								  <OPTION VALUE="May"><?php $style->textout($language[May]); ?></OPTION>
								  <OPTION VALUE="June"><?php $style->textout($language[June]); ?></OPTION>
								  <OPTION VALUE="July"><?php $style->textout($language[July]); ?></OPTION>
								  <OPTION VALUE="August"><?php $style->textout($language[August]); ?></OPTION>
								  <OPTION VALUE="September"><?php $style->textout($language[September]); ?></OPTION>
								  <OPTION VALUE="October"><?php $style->textout($language[October]); ?></OPTION>
								  <OPTION VALUE="November"><?php $style->textout($language[November]); ?></OPTION>
								  <OPTION VALUE="December"><?php $style->textout($language[December]); ?></OPTION>
								  </select> <?php $style->textout("/"); ?> 
								  
								  <select name=year>
								  <?php
								  $start = date("Y");
								  $stop = $start - 150; // people gona be over 100 ?
								 ...
								 ?>
									...
									<?php $style->textout("eg. 21/03/2002"); ?></td>
...
<?php $style->headertextout($language[optionalinfo]); ?>
                           ...
                           <tr>
                                   ...
                                   <td bgcolor="<?php echo $style->tbl2color; ?>"><textarea name=signiture cols=30 rows=4></textarea></td>
...
?>
						   ...
						   <tr>
                                   ...
                                   <td bgcolor="<?php echo $style->tbl1color; ?>"><b><?php $style->textout($language[avatar] .""); ?></b></td>

                                   <td bgcolor="<?php echo $style->tbl2color; ?>">
								   <table cellpadding=0 cellspacing=0 border=0 bgcolor='' width=100%><tr><td width=50%>
								   
								   <input type=text name=image size=30 maxlength=80> 
								   <br>
								   <br>
...
function init(){
document.signup.images.options[0].selected = true;
...
<img name='switchimg' src='<?php print "$avatar_url/spacer.gif"; ?>' border='0' width='50' height='50'>									  
</td>
</tr>
</table>
</td>
...
special_title, notifytype,avatar, birthday)

VALUES (

'', '$u_name',  '$p_word', '2', '$ip', '$reg_date',  '', '', '$email', '$viewemail', '$icq', '$aim', '$msnm', '$interests', '$signiture', '', '', '', '$homepage', 'n', '','$image', '$birthday')
...
<td bgcolor=<?php echo $style->tbl1color; ?> align=center><?php $style->textout("$language[thanx] <b>$u_name</b> $language[forsign] !"); ?>
...
bbactivation("memberscontactact");
...
<?php $style->headertextout("$language[ccentre] $username"); ?></th>
...
<?php $style->headertextout($language[fillit]); ?></th>

</tr><tr>

<td width="20%" bgcolor="<?php echo $style->tbl1color; ?>" >

<?php $style->textout("$language[contact] $username :"); ?><br><br>



<form action="members.php?do=message" method="POST">

<?php $style->textout("$language[selecttypes] :<br>"); ?>
...
print "<option value='sendmail'>$language[email]</option>";
...
<?php $style->textout("$language[theemail] : $addy"); ?><br>

</td></tr><tr><td>

<?php $style->textout($language[message].":"); ?><br>
...
<input type=submit value=<?php echo $language[submit]; ?>></form>
...
bbactivation("membersviewact");


$info = $stream->do_query("select id, username, rank, reg_date, num_posts, email, icq, msnm, aim, awards, views, homepage, avatar, birthday from evo_users where id = '$member'", "array");
...
$birthday = $tmp[13];
$shit = explode(",",$birthday);
$month = checkmonth($shit[1]);
$birthday = "$shit[0],$month,$shit[2]";



$adminpanelset = $stream->do_query("SELECT membersrank from evo_settings where id='1'", "one"); 

if($rank<=$adminpanelset){
...
<td width="25%" bgcolor="<?php echo $style->tbl1color; ?>" ><?php $style->textout($language[birthday]);?></td>

<td width=* bgcolor="<?php echo $style->tbl2color; ?>"  align=right><?php $style->textout("$birthday"); ?>&nbsp;</td>
...
<td width=100% colspan=2 bgcolor="<?php echo $style->tbl_header; ?>"><?php $style->headertextout($language[statistics]); ?></td>
...
<td width="25%" bgcolor="<?php echo $style->tbl1color; ?>" ><?php $style->textout($language[avatar]);?></td>

<td width=* bgcolor="<?php echo $style->tbl2color; ?>"  align=right><?php $style->textout("<img src='./images/avatars/$avatar' width=50 height=50>"); ?>&nbsp;</td>
...
<td width=100% colspan=2 bgcolor="<?php echo $style->tbl_header; ?>"><?php $style->headertextout($language[actions]); ?></td>

</tr>

<tr>

<td width="25%" bgcolor="<?php echo $style->tbl1color; ?>" ><?php $style->textout($language[contact]); ?> <?php $style->textout("$username"); ?> :</td>
...
<td width="25%" bgcolor="<?php echo $style->tbl1color; ?>" >  <?php $style->textout($language[FYI]); ?> </td>

<td width=* bgcolor="<?php echo $style->tbl2color; ?>" align=left>   <?php $style->textout($language[omitted]); ?></td>
...
<td align=center bgcolor="<?php echo $style->tbl1color; ?>" colspan=8>

<?php $style->textout($language[omitted]); ?>
