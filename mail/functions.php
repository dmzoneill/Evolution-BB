...
function checkBlocked($string) {
	global $username;
	$filename = "users/".$username."/blocklist.conf";
	$fp       = fopen($filename, "r");
	$buffer   = fread($fp, filesize($filename));
	$address  = explode(",", $buffer);
    for($x=0;$x<count($address);$x++) {
	    	if(ereg($address[$x], $string)) {
            	return 1;
            }
    }
        fclose($fp);
...
function updateBlockedList($string) {
	global $username;
	$filename = "users/".$username."/blocklist.conf";
	$fp       = fopen($filename, "r");
    fwrite($fp, $string);
...
if (!in_array(basename($SCRIPT_NAME), $noConnectionNeeded)) {
	...
	if ($host && $port && $username && $password && (!$remoteAddress || $REMOTE_ADDR == $remoteAddress)) {
		...
		echo "<input type='hidden' name='host' value='evobb.com'>";
