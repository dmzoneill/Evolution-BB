...
$SESS_DBNAME = "todo_dev2";			/* database name */
...
function sess_read($key) {
	...
	$qry = "SELECT value FROM todo_sessions WHERE sesskey = '$key' AND expiry > " . time();
...
function sess_write($key, $val) {
	...
	$qry = "INSERT INTO todo_sessions VALUES ('$key', $expiry, '$value')";
	$qid = mysql_query($qry, $SESS_DBH);

	if (! $qid) {
		$qry = "UPDATE todo_sessions SET expiry = $expiry, value = '$value' WHERE sesskey = '$key' AND expiry > " . time();
		$qid = mysql_query($qry, $SESS_DBH);
	}
...
function sess_destroy($key) {
	...
	$qry = "DELETE FROM todo_sessions WHERE sesskey = '$key'";
	$qid = mysql_query($qry, $SESS_DBH);
...
function sess_gc($maxlifetime) {
	...
	$qry = "DELETE FROM todo_sessions WHERE expiry < " . time();
	$qid = mysql_query($qry, $SESS_DBH);
