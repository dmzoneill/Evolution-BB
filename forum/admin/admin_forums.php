...
Revision: 0.3
	- forgot to add moderating changes
*/ 
...
switch ($do) { 
  ...
  $result = $stream->do_query("select auto, fgroup, title, description, mods, last, rules, type from evo_forums where auto = '$id'", "row"); 
	...
	<td><input type="text" name="description" value="<?php echo rawurldecode($description); ?>" size="20"></td> 
 	</tr> 
 	<td><b>Moderators:</b></td>  
	<td><a href=admin_forums.php?do=editmod&forumid=<?php echo $id; ?>>Edit Moderators</a></td> 
 	</tr> 
 	</table> 
	 <br> 
	<? 
...
case "editmod":
	$result = $stream->do_query("select mods from evo_forums where auto = '$forumid'", "one");
	?>
		<table width=80% border=0 cellpadding=0 cellspacing=0>
		<tr><td bgcolor=black>
			<table width=100% border=0>
			<tr bgcolor=#ffffff> 
				<th colspan=3>Remove Moderators</th>
			</tr>
			<tr bgcolor=#ffffff>
			...
			<form method=post action="admin_forums.php?do=dodelmod&forumid=<?php echo $forumid; ?>">
	<?
	$result = explode("|", $result);
	for($i=0; $i<count($result); $i++)  {
		$tmpUsername = $stream->do_query("select username from evo_users where id = '$result[$i]'", "one");
		$tmpId            = $result[$i];
		echo "<tr><td bgcolor=#B3CAFF>$tmpId</td><td bgcolor=#B3CAFF>$tmpUsername</td><td bgcolor=#B3CAFF><input type=checkbox name=mods[] value=$tmpId></td></tr>";
	}
	?>
		</table>
		</tr></td></table>		
		<input name="go" value="Remove" type="submit">		</form>
		<br><br>
		<form method=post action=admin_forums.php?do=doaddmod&forumid=<?php echo $forumid; ?>>
		Add Moderator:&nbsp;&nbsp; <select name=mod>
	<?
	$users = returnUserArry();
	...
	?>
		</select> <input name="submit" value="Add" type="submit"> </form>
	<?
...
case "doaddmod":
	if($stream->do_query("select rank from evo_users where id = '$mod'", "one")<3) {
		$name = id2name($mod);
		echo "Because the user: <b>$user</b> was not currently a moderator, I have set that for you. \n <br>";
		$result = $stream->do_query("update evo_users set rank = '3' where id = '$mod'", "one");
	}
	$previous = $stream->do_query("select mods from evo_forums where auto = '$forumid'", "one");
	if($previous=="0") { $a = $stream->do_query("update evo_forums set mods = '$str' where auto = '$forumid'", "one"); }
	if($previous=="bad") { die("Unable to query db to grab pervious string"); }
	$str = "$previous|$mod";
	$result = $stream->do_query("update evo_forums set mods = '$str' where auto = '$forumid'", "one");
	if($result=="bad") { die("Unable to query db to insert new mods"); }
...
case "dodelmod":
	...
	// Populate a new array to get rid of the "" in $result
	$newResult = array();
	$y=0;
	for($i=0;$i<count($result);$i++) {
		if($result[$i]!="")  {
			$newResult[$y] = $result[$i];
			$y++;
		}
	}
	$newResult = implode("|", $newResult);
	$sql = $stream->do_query("update evo_forums set mods = '$newResult' where auto = '$forumid'", "one");
	if($sql=="bad") {
		die("Couldn't update table evo_forums");
	} else {
